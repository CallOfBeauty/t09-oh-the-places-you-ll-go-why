...
# Author: Natinael Abebe and Dimitrios Ntentia     TODO: Change this to your names
# Username: abeben, ntentiad                    TODO: Change this to your usernames
#
...
def parse_file(filename):
    ...
    for i in range(str_num):
        ...
        places_list.append(extract_place(file_content,filename))         # Assembles the list of places
...
def extract_place(file_content, filename):
    ...
    # TODO   (Extra) Once you've got the code working for all five lines, add a conditional that checks to see if
    ...
    # FIXME Construct a tuple with all five values in the correct order. Don't forget types, and tuples are immutable!
    # Example: place_tuple = ("Scott's example", "Somewhere special", 41, -10, "black")
    openedfile = open(filename, 'r')  # Opens file for reading
    str_file = openedfile.readline()
    str_file = int(str_file[:-1])
    place_list=[]
    for i in range(10):
        str_file = openedfile.readline()
        place_list.append(str_file)
    openedfile.close()
    counter=1
    for item in range(len(place_list)):
        if counter%3==0 or counter%4==0:
            float(place_list[item])
            counter+=1
            if counter==5:
                counter=0
    place_tuple = (i for i in place_list)
